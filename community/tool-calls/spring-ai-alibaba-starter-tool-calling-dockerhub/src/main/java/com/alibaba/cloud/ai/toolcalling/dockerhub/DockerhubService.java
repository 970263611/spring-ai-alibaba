...
public class DockerhubService
		implements Function<DockerhubService.Request, DockerhubService.Response> {

	private final RestClientTool restClientTool;

	public DockerhubService(RestClientTool restClientTool) {
	...
	@JsonClassDescription("get response from dockerhub search api")
	public record Request(@JsonPropertyDescription("Search model name") String modelName) {
	}

	@JsonClassDescription("dockerhub search api result")
